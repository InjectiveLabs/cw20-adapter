...
mod tests {
    ...
    use cosmwasm_std::{testing::{mock_info, mock_env}, SubMsg, QuerierResult, SystemResult, SystemError};
    use injective_cosmwasm::{mock_dependencies, InjectiveRoute, InjectiveMsg, WasmMockQuerier, HandlesFeeQuery};
    ...
    fn it_handles_correct_register_msg_with_exact_funds() {
        ...
        let response = handle_register_msg(deps.as_mut(), env, mock_info(SENDER, &vec![Coin::new(10, "inj")]), Addr::unchecked(CW_20_ADDRESS)).unwrap();
        ...
        if let SubMsg{ msg: CosmosMsg::Custom(InjectiveMsgWrapper{ route, msg_data }), .. } = response.messages.first().unwrap() {
            ...
            if let InjectiveMsg::CreateDenom{ sender, subdenom } = msg_data {
                ...
                assert_eq!(CW_20_ADDRESS, subdenom.as_str(), "incorrect subdenom n the create denom message");
            } else {
    ...
    fn it_handles_correct_register_msg_with_extra_funds() {
        ...
        let mut env = mock_env();
        env.contract.address = Addr::unchecked(CONTRACT_ADDRESS);
        let response = handle_register_msg(deps.as_mut(), env, mock_info(SENDER, &vec![Coin::new(100, "inj"), Coin::new(20, "usdt")]), Addr::unchecked(CW_20_ADDRESS)).unwrap();
    ...
    fn it_handles_correct_register_msg_with_non_cannonical_cw20_address() {
        ...
        let response = handle_register_msg(deps.as_mut(), mock_env(), mock_info(SENDER, &vec![Coin::new(10, "inj")]), Addr::unchecked(non_cannonical_address.clone())).unwrap();

        let contract_registered = CW20_CONTRACTS.contains(&deps.storage, non_cannonical_address.clone());
        assert!(contract_registered, "contract wasn't registered");
        ...
        if let SubMsg{ msg: CosmosMsg::Custom(InjectiveMsgWrapper{ route, msg_data }), .. } = response.messages.first().unwrap() {
            ...
            if let InjectiveMsg::CreateDenom{ sender, subdenom } = msg_data {
                ...
                assert_eq!(non_cannonical_address, subdenom.as_str(), "incorrect subdenom n the create denom message");
            } else {
                panic!("incorrect injective message found")
            }
    ...
    fn it_returns_error_if_already_registered_register_msg() {
        ...
        assert_eq!(response.unwrap_err(), ContractError::ContractAlreadyRegistered, "incorrect error returned")
    }
    ...
    fn it_returns_error_if_cannot_query_denom_creation_fee_register_msg() {
        ...
        querier.token_factory_denom_creation_fee_handler = create_denom_creation_fee_failing_handler();
    ...
    fn it_returns_error_if_mismatched_denom_is_passed_register_msg() {
        let mut deps = mock_dependencies();
        let response = handle_register_msg(deps.as_mut(), mock_env(), mock_info(SENDER, &vec![Coin::new(10, "usdt")]), Addr::unchecked(CW_20_ADDRESS)).unwrap_err();
    ...
    fn it_returns_error_if_insufficient_coins_are_passed_register_msg() {
        let mut deps = mock_dependencies();
        let response = handle_register_msg(deps.as_mut(), mock_env(), mock_info(SENDER, &vec![Coin::new(9, "inj")]), Addr::unchecked(CW_20_ADDRESS)).unwrap_err();
    ...
    fn it_returns_error_if_no_coins_are_passed_register_msg() {
        let mut deps = mock_dependencies();
        let response = handle_register_msg(deps.as_mut(), mock_env(), mock_info(SENDER, &vec![]), Addr::unchecked(CW_20_ADDRESS)).unwrap_err();
    ...
    fn it_handles_receive_correctly_if_not_already_registered() {
        ...
        let response = handle_on_received_cw20_funds_msg(deps.as_mut(), mock_env(), mock_info(SENDER, &vec![Coin::new(10, "inj")]), CW_20_ADDRESS.to_string(), amount).unwrap();
    ...
    #[test]
    fn it_returns_error_on_receive_if_contract_not_registered_and_mismatched_denom_is_passed() {

    }

    #[test]
    fn it_returns_error_on_receive_if_contract_not_registered_and_insufficient_coins_are_passed() {
    ...
    fn create_denom_creation_fee_failing_handler() -> Option<Box<dyn HandlesFeeQuery>> {
        ...
        impl HandlesFeeQuery for Temp {
            fn handle(&self) -> QuerierResult {
                SystemResult::Err(SystemError::UnsupportedRequest { kind: "custom error".to_string() })
            }
        }
        Some(Box::new(Temp ()))
    }
